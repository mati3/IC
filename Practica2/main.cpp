// g++ ./main.cpp -O2
// ./a.out datos/chr12c.dat 20 10000 normal
...
////////////////////////////////////////////////////////////////////////////
void reemplazo(vector<int> & mejor_individuo,  vector<int> & fitness, vector<vector<int> > & popu, vector<vector<int> > & hijos){
...
// srand (time(NULL)); // quitar semilla, para que sea todo aleatorio
	...
	if (opcion == "normal"){
	   ...
	   for(int p = 0 ; p < iteraciones ; p++){
		...
		// cruzo los seleccionados
		vector<vector<int> > hijos;
		hijos.reserve(seleccionados.size());
		for(int i = 0 ; i < seleccionados.size()-1; i++){
			hijos.push_back(cruce(poblacion[seleccionados[i]],poblacion[seleccionados[i+1]]));	
		}
		// cruzo el último con el primero
		hijos.push_back(cruce(poblacion[seleccionados.size()-1],poblacion[seleccionados[0]]));
		...
		reemplazo(mejor_individuo,allFit,poblacion,hijos);
		hijos.clear();
	   ...
	   for(int p = 0 ; p < iteraciones ; p++){
		...
		// muestro el mejor fitness y guardo el mejor individuo
		if (p%10 == 0){
			cout  << ","<< mejorfitness(mejor_individuo,allFit,poblacion) ;
		}else{
			mejorfitness(mejor_individuo,allFit,poblacion);
